...
func (s *sendMail) SendResetPassword(ToUser string, FromAdmin string, Token string, Sendgridkey string, AppEnv string) (*EmailResponse, error) {
	h := hermes.Hermes{
		Product: hermes.Product{
			Name: "SeamFlow",
			Link: os.Getenv("FRONT_END_BASE_UR"),
		},
	}
	var forgotUrl string
	if os.Getenv("APP_ENV") == "production" {
		forgotUrl = os.Getenv("FRONT_END_BASE_UR") + Token //this is the url of the frontend app
	} else {
		forgotUrl = os.Getenv("FRONT_END_BASE_UR") + Token //this is the url of the local frontend app
	}
	email := hermes.Email{
		Body: hermes.Body{
			Name: ToUser,
